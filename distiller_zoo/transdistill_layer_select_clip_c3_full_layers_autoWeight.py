    ...
    def forward(self, anchor, logit_loss):
        ...
        for i, ty in enumerate(layer_type_list):
            (
                ...
                tea_vv_list,
                stu_vv_list,
            ) = self._preprocess_qkv_layers(state=ty, fea_dict=anchor, w_softmax=True)

            for j in range(len(tea_qk_list)):
                tea_qk, stu_qk, tea_vv, stu_vv = (
                    tea_qk_list[j],
                    stu_qk_list[j],
                    tea_vv_list[j],
                    stu_vv_list[j],
                ...
                # The index of param
                qk_idx = i * len(tea_qk_list) + j + 1
                vv_idx = (i * len(tea_qk_list) + j + 1) + self.alpha

                loss = sm_params[qk_idx] * self.Tem**2 * self.kl_div(
                    stu_qk.log(), tea_qk
                ) + sm_params[vv_idx] * self.Tem**2 * self.kl_div(
                    stu_vv.log(), tea_vv
                )
...
if __name__ == "__main__":
    sad_kl_Loss = Loss(temperature=1, alpha=12)
    print(sad_kl_Loss.state_dict())
    ##################################33
    # K = 128
    # tea_dim = 512
    # stu_dim = 256
