...
class Loss(nn.Module):
    ...
    def __init__(self, state1, layer1, state2, layer2, state3, layer3, temperature, alpha):
        ...
        self.linear = nn.Linear(int(512), int(200)).cuda()
    ...
    def _preprocess_fitnet_star(self, fea_dict):
        stu_fea, tea_fea = fea_dict['stu_fea'], fea_dict['tea_fea']

        f_t = tea_fea['Fus_Trans']['mem_emb'][self.fitnetlayer]
        # print(f"f_t: {f_t.shape}")
        f_t = self.linear(f_t)
        f_s = stu_fea['Fus_Trans']['mem_emb'][self.fitnetlayer]

        f_t_av = self.linear(tea_fea['AV_Trans']['av_emb'][self.fitnetlayer])
        f_s_av = stu_fea['AV_Trans']['av_emb'][self.fitnetlayer]

        f_t_va = self.linear(tea_fea['AV_Trans']['va_emb'][self.fitnetlayer])
        f_s_va = stu_fea['AV_Trans']['va_emb'][self.fitnetlayer]
    
        return f_s_av, f_t_av, f_s_va, f_t_va, f_s, f_t
    ...
    def forward(self, anchor):
        ...
        f_s_av, f_t_av, f_s_va, f_t_va, f_s, f_t = self._preprocess_fitnet_star(anchor)
        ...
        fitnet_star_loss = self.crit(f_s_av, f_t_av) + self.crit(f_s_va, f_t_va) + self.crit(f_s, f_t)
        
        return self.alpha * (loss1 + loss2 + loss3), fitnet_star_loss
