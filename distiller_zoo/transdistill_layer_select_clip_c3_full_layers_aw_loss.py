    ...
    def forward(self, anchor, logit_loss):
        ...
        for i, layer_type in enumerate(layer_types):
            ...
            for j, (tea_qk, stu_qk, tea_vv, stu_vv) in enumerate(zip(
                ...
                vv_idx = i * len_tchr_list + j + 1 + self.alpha
                
                # 原始的 qk_loss 和 vv_loss
                qk_loss = self.Tem**2 * self.kl_div(stu_qk.log(), tea_qk)
                vv_loss = self.Tem**2 * self.kl_div(stu_vv.log(), tea_vv)
                
                # 加權的 qk_loss 和 vv_loss
                qk_loss = 0.5 / (self.params[qk_idx] ** 2) * (
                    qk_loss
                ) + torch.log(1 + self.params[qk_idx] ** 2)
                
                vv_loss = 0.5 / (self.params[vv_idx] ** 2) * (
                    vv_loss
                ) + torch.log(1 + self.params[vv_idx] ** 2)
                
                loss_sum += qk_loss + vv_loss
